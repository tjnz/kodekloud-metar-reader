...
def parse_metar(raw):
    ...
    if idx < len(tokens):
        ...
        if wm:
            raw_dir, raw_spd, _, raw_gust, unit = (
                wm.group(1),
                wm.group(2),
                wm.group(3),
                wm.group(4),
    ...
    if idx < len(tokens) and tokens[idx] == "CAVOK":
        ...
        result["details"].append(
            {"icon": "👁️", "label": "Visibility", "value": "10+ km (CAVOK)"}
        )
        result["details"].append(
            {"icon": "🌤️", "label": "Sky", "value": "Ceiling and visibility OK"}
        )
        idx += 1
    ...
    # Visibility
        ...
        if m:
            ...
            result["details"].append(
                {"icon": "👁️", "label": "Visibility", "value": vis_txt}
            )
            idx += 1
        elif mf:
    ...
    if sky_layers:
        ...
        result["details"].append(
            {"icon": "🌤️", "label": "Sky", "value": "; ".join(sky_parts)}
        )
    ...
    if idx < len(tokens):
        ...
        if am:
            ...
            result["details"].append(
                {"icon": "📊", "label": "Pressure", "value": f"{alt:.2f} inHg"}
            )
            idx += 1
        elif qm:
            hpa = int(qm.group(1))
...
def _build_summary(d):
    ...
    if d["wind"]:
        ...
        if w["speed_kt"] == 0:
            ...
            parts.append(
                f"winds {w['speed_mph']}–{w['gust_mph']} mph from the {w['direction']}"
            )
        else:
